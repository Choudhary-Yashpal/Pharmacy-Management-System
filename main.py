...
root = Tk()
root.title("simple pharmacy Managmnent System")
root.configure(width=1500,height=600,bg="purple2")
var=-1
def additem():
    global var
    num_lines = 0
    ...
    with open(r"database_proj") as f1, open(r"database_proj1", "w") as working:
        for line in f1:
            if str(e1) not in line:
                ...
                working.write('{ø} {1} {2} {3} {4}'.format(str(e1), e2, e3, str(e4), e5))
    os.remove(r"database_proj")
...
def searchitem():
    i=0
    ell = entry1.get()
    with open(r"database proj") as working:
        for line in working:
            i-1+1
            if str(ell) in line:
               break
...
def clearitem():
    ...
    entry5.delete(0, END)
#fn1353
label0= Label(root,text="PHARMACY MANAGEMENT SYSTEM",bg="#f4511e",fg="black",font=("times",30))
...
button1=Button(root, text="ADD ITEM",bg="white", fg="black", width=0, font=("Tines", 12), command=additem)
